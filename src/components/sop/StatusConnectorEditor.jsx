...
import {
  ...
  AlertTriangle, CheckCircle, GitBranch, Pause, XCircle
} from 'lucide-react'
import {
  STATUS_NODE_TYPES, STATUS_NODE_COLORS, CONNECTOR_TYPES,
  DEFAULT_STATUS, DEFAULT_CONNECTOR,
} from './sopConstants'
...
const StatusConnectorEditor = ({ visualState, onChange, availableSOPs = [], onNavigateToSOP }) => {
  ...
  const [activeSection, setActiveSection] = useState('statuses') // 'statuses' | 'connectors'
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <div>
                          ...
                          <input type="text" value={conn.assignee}
                            onChange={e => updateConnector(idx, 'assignee', e.target.value)}
                            placeholder="Who performs this action?"
                            className={inp} />
                        </div>
                        <div>
