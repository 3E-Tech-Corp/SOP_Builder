...
import { Plus, Edit3, Copy, Trash2, Play, Download, Upload, Workflow, Clock, GitBranch, Database } from 'lucide-react';
import { loadSOPs, saveSOP, deleteSOP, duplicateSOP, exportSOP, importSOP } from '../utils/storage';
import useSopStore from '../store/sopStore';

export default function SopList() {
  const [sops, setSOPs] = useState(() => loadSOPs());
  ...
  const refresh = () => setSOPs(loadSOPs());
  ...
  const handleImport = async (e) => {
    ...
    if (!file) return;
    try {
      const sop = await importSOP(file);
      refresh();
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">Design, test, and manage your Standard Operating Procedures</p>
          </div>
            ...
            <div className="bg-slate-800 border border-slate-600 rounded-xl shadow-2xl p-6 w-full max-w-md mx-4">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input type="text" value={newName} onChange={e => setNewName(e.target.value)}
                    placeholder="e.g., Employee Onboarding" autoFocus onKeyDown={e => e.key === 'Enter' && handleCreate()}
                ...
                <div>
                  ...
                  <textarea value={newDesc} onChange={e => setNewDesc(e.target.value)}
                    placeholder="Brief description of this SOP..." rows={3}
              ...
              <div className="flex justify-end gap-2 mt-5">
                <button onClick={() => { setShowCreate(false); setNewName(''); setNewDesc(''); }}
                  className="px-4 py-2 text-sm text-slate-300 hover:text-white transition-colors">Cancel</button>
...
function SopCard({ sop, onEdit, onTest, onDuplicate, onDelete, onExport }) {
  const nodeCount = sop.nodes?.length || 0;
  const edgeCount = sop.edges?.length || 0;
  const propCount = sop.objectSchema?.properties?.length || 0;
    ...
    <div className="group bg-slate-800 border border-slate-700 rounded-xl p-5 hover:border-synthia-500/50 transition-all hover:shadow-lg hover:shadow-synthia-500/5">
      ...
      <div className="flex items-center gap-1.5">
        <button onClick={onEdit}
          ...
          <Edit3 className="w-3 h-3" /> Design
        </button>
